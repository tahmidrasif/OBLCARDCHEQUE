    ...
    public class RequisitionController : Controller
    {
        ...
        public ActionResult Index(int? STATUS, string CARDNO, DateTime? CREATEDON, int? page)
        {
            try
            {
                ...
                var List = db.CARDCHEREUISITION.Include(c => c.BRANCHINFO).OrderByDescending(x => x.CREATEDON).ToList();
        ...
        public ActionResult Details(long? id)
        {
            try
            {
                ...
                ViewBag.BRANCH = new SelectList(db.BRANCHINFO, "ID", "BRANCHNAME", requisition.BRANCHCODE);

                ViewBag.STATUSID = new SelectList(db.OCCENUMERATION.Where(x => x.Type == "chequereq"), "ID", "Name", requisition.STATUS);
